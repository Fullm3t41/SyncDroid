...
class WorkerControlTest {
    ...
    fun workerFileLockAllowsOnlyOneOwner() {
        val path = Files.createTempDirectory("syncdows-worker-lock").resolve("worker.lock")
    ...
    @Test
    fun developmentUiLaunchUsesTheUiEntryPoint() {
        val command = AppProcessLauncher.command(UI_ARGUMENT)

        assertTrue(command.contains("com.syncdows.app.MainKt"))
        assertEquals(UI_ARGUMENT, command.last())
    }
}
