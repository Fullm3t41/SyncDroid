...
class MeshStoreTest {
    ...
    fun createdMeshRoundTripsThroughAndroidBundleFormat() {
        val directory = Files.createTempDirectory("syncdows-store-test")
        val signer = signer()
    ...
    fun tamperedChatMessageIsRejected() {
        val directory = Files.createTempDirectory("syncdows-chat-test")
        val signer = signer()
    ...
    fun importsLegacyAndroidCreatorThroughAJoinedOfferingDevice() {
        val directory = Files.createTempDirectory("syncdows-android-compat-test")
        val creator = signer()
    ...
    fun rejectsPairingFromADeviceRemovedFromTheMesh() {
        val directory = Files.createTempDirectory("syncdows-removed-peer-test")
        val creator = signer()
    ...
    fun androidFolderAnnouncementArrivesPendingAndCanBeConfigured() {
        val directory = Files.createTempDirectory("syncdows-folder-import-test")
        val signer = signer()
    ...
    @Test
    fun windowsCanCreateAndExportAFilteredMeshFolder() {
        val directory = Files.createTempDirectory("syncdows-folder-create-test")
        val signer = signer()
        MeshStore(directory.resolve("source.db")).use { source ->
            val profile = source.createMesh("Home mesh", "Windows PC", signer)
            val announcement = FolderAnnouncement.create(
                profile.groupId,
                "Game saves",
                listOf(" *.sav ", "Saves/**", "*.sav"),
                listOf("Cache/**"),
                signer,
                createdAtMillis = 1_700_000_000_100L,
            )
            assertTrue(source.applyFolder(announcement))
            assertEquals(listOf("*.sav", "Saves/**"), announcement.includePatterns)
            assertEquals(listOf("Cache/**"), announcement.excludePatterns)

            MeshStore(directory.resolve("destination.db")).use { destination ->
                destination.importBundle(MeshWireCodec.decode(MeshWireCodec.encode(source.exportBundle())))
                val received = destination.folders(profile.groupId, signer.deviceId).single()
                assertEquals("Game saves", received.displayName)
                assertEquals(LocalFolderBindingState.PENDING_CONFIGURATION, received.bindingState)
            }
        }
    }
    ...
    fun signedOverwriteOnlyExceptionsReplicateAndCanBeUndone() {
        val directory = Files.createTempDirectory("syncdows-exception-test")
        val signer = signer()
        MeshStore(directory.resolve("source.db")).use { source ->
            val profile = source.createMesh("Home mesh", "Mac", signer)
            ...
            assertEquals(listOf("save/main.sav"), source.activeSyncExceptions().map { it.relativePath })
            ...
            assertTrue(source.activeSyncExceptions().isEmpty())
        }
    }

    @Test
    fun signedRenameRemovalAndLocalMeshClearAreApplied() {
        val directory = Files.createTempDirectory("syncdows-membership-management-test")
        val owner = signer()
        val peer = signer()
        MeshStore(directory.resolve("mesh.db")).use { store ->
            val profile = store.createMesh("Home mesh", "Windows PC", owner)
            val created = store.membershipEvents(profile.groupId)
            val add = MembershipEvent.createAddDevice(
                profile.groupId, "Android", peer.publicKey, owner,
                created.map(MembershipEvent::eventId), VersionVector(mapOf(owner.deviceId to 2)), 100,
            )
            assertTrue(store.applyMembership(profile.groupName, add))
            val rename = MembershipEvent.createUpdateDeviceName(
                profile.groupId, "Desktop", owner.publicKey, owner,
                listOf(add.eventId), VersionVector(mapOf(owner.deviceId to 3)), 200,
            )
            assertTrue(store.applyMembership(profile.groupName, rename))
            assertEquals("Desktop", store.devices(profile.groupId).single { it.deviceId == owner.deviceId }.displayName)

            val remove = MembershipEvent.createRemoveDevice(
                profile.groupId, "Android", peer.publicKey, owner,
                listOf(rename.eventId), VersionVector(mapOf(owner.deviceId to 4)), 300,
            )
            assertTrue(store.applyMembership(profile.groupName, remove))
            assertEquals(false, store.devices(profile.groupId).single { it.deviceId == peer.deviceId }.trusted)

            store.clearMesh()
            assertEquals(null, store.profile())
            assertTrue(store.membershipEvents(profile.groupId).isEmpty())
            assertTrue(store.devices(profile.groupId).isEmpty())
        }
    }
