...
data class MembershipEvent(
     ...
     * authorize a new SyncDows member without rewriting its immutable history.
     */
    ...
    companion object {
        ...
        fun createUpdateDeviceName(
            ...
            groupId, MembershipEventType.UpdateDeviceName, displayName, subjectPublicKey,
            signer, parentEventIds, version, createdAtMillis,
        )
...
data class FolderAnnouncement(
    ...
    companion object {
        fun create(
            ...
            signer: DeviceSigner,
            version: VersionVector = VersionVector().increment(signer.deviceId),
            folderId: String = UUID.randomUUID().toString(),
            createdAtMillis: Long = System.currentTimeMillis(),
        ): FolderAnnouncement {
            val cleanName = displayName.trim()
            require(cleanName.isNotEmpty() && cleanName.length <= 128) { "Enter a folder name" }
            val includes = normalizePatterns(includePatterns)
            val excludes = normalizePatterns(excludePatterns)
            ...
            return FolderAnnouncement(
                ...
                version,
                createdAtMillis,
                Base64.getEncoder().encodeToString(signer.sign(payload)),
            )
        }
    }
...
private fun normalizePatterns(patterns: List<String>): List<String> = patterns
    ...
    .onEach { require(it.length <= 512) { "A folder filter is too long" } }
    .distinct()
    .sorted()
