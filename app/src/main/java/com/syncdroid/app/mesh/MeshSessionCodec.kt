...
object MeshSessionCodec {
        ...
        DataOutputStream(bytes).use { output ->
            ...
            when (message) {
                is MeshSessionMessage.Metadata -> {
                    ...
                    output.writeBytes(message.bundle)
                }
    ...
    fun decode(bytes: ByteArray): MeshSessionMessage = DataInputStream(ByteArrayInputStream(bytes)).use { input ->
        require(input.readInt() == MAGIC && input.readUnsignedShort() == VERSION) { "Unsupported mesh session protocol" }
        val message = when (val type = input.readUnsignedByte()) {
            METADATA -> MeshSessionMessage.Metadata(input.readBytes())
    ...
    private fun DataInputStream.readString() = String(readBytes(), StandardCharsets.UTF_8)
    ...
    private fun DataInputStream.readBytes(): ByteArray = ByteArray(readInt().also {
    ...
    private fun DataInputStream.readCount(): Int = readInt().also { require(it in 0..MAX_ITEMS) }
