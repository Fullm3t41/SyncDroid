...
class MeshRuntime(
        ...
        scope.launch {
            ...
            appInForeground.collectLatest { foreground ->
                if (shouldRunContinuousDiscovery(foreground, scheduledDiscoveryEnabled)) {
                    ...
                    // Cancelling this branch closes the active NSD window immediately.
                    ...
                    while (isActive) {
                        val delayMillis = millisUntilNextRendezvous(LocalDateTime.now(), rendezvousIntervalMinutes)
    ...
    private suspend fun runDiscoveryWindow(client: MeshPeerClient, port: Int, durationMillis: Long?) {
        ...
        val collector = scope.launch {
            nsd.peers.collect { peers ->
                ...
                removed.forEach { id -> peerJobs.remove(id)?.cancel() }
    ...
    private suspend fun runSessionOnce(
        ...
        if (!activePeers.add(connection.peer.deviceId)) return
        ...
        try {
            ...
            val syncedFolders = database.syncDao().configuredBindings(identity.deviceId)
            ...
            activePeers.remove(connection.peer.deviceId)
        }
    }
